...
If what we are looking for is a model that minimizes both false positives and false negatives, we should opt for the XGB algorithm <code> rf.Fit1 in the code </code>, with the set of original characteristics added to those we build, in addition to taking care of the resampling technique we perform use to train the algorithm 